...
(function () {
    ...
    app.component('editProspect', {
        templateUrl: '/content/js/app/admin/prospect/edit-prospect.html',
        controller: editProspectController,
        bindings: {
            resolve: '<',
            close: '&',
            dismiss: '&',
    ...
    editProspectController.$inject = ['prospectService', 'adminUtility'];
    function editProspectController(prospectService, adminUtility) {
        var $ctrl = this;

        $ctrl.save = function () {
            adminUtility.validateForm($ctrl.addEditForm);
            if ($ctrl.working.id) {
                prospectService.update($ctrl.working).then(function (response) {   // Update 
                    $ctrl.close({ $value: $ctrl.working });
                }, processErrors)
            } else {
                prospectService.create($ctrl.working).then(function (response) {   // Create 
                    $ctrl.close({ $value: $ctrl.working });
                }, processErrors)
            }
        };
