...
(function () {
    ...
    app.component('editPresenter', {
        templateUrl: '/content/js/app/admin/presenter/edit-presenter.html',
        controller: editPresenterController,
        bindings: {
            resolve: '<',
            close: '&',
            dismiss: '&',
    ...
    editPresenterController.$inject = ['presenterService', 'adminUtility'];
    function editPresenterController(presenterService, adminUtility) {
        var $ctrl = this;

        $ctrl.save = function () {
            adminUtility.validateForm($ctrl.addEditForm);
            if ($ctrl.working.id) {
                presenterService.update($ctrl.working).then(function (response) {   // Update 
                    $ctrl.close({ $value: $ctrl.working });
                }, processErrors)
            } else {
                presenterService.create($ctrl.working).then(function (response) {   // Create 
                    $ctrl.close({ $value: $ctrl.working });
                }, processErrors)
            }
        };
