...
(function () {
    ...
    app.component('editTimeOfDay', {
        templateUrl: '/content/js/app/admin/time-of-day/edit-time-of-day.html',
    ...
    editTimeOfDayController.$inject = ['timeOfDayService', 'adminUtility'];
    function editTimeOfDayController(timeOfDayService, adminUtility) {
        var $ctrl = this;

        $ctrl.save = function () {
            adminUtility.validateForm($ctrl.addEditForm);
            if ($ctrl.working.id) {
                timeOfDayService.update($ctrl.working).then(function (response) {   // Update 
                    $ctrl.close({ $value: $ctrl.working });
                }, processErrors)
            } else {
                timeOfDayService.create($ctrl.working).then(function (response) {   // Create 
                    $ctrl.close({ $value: $ctrl.working });
                }, processErrors)
            }
        };
