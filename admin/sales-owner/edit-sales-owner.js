...
(function () {
    ...
    app.component('editSalesOwner', {
        templateUrl: '/content/js/app/admin/sales-owner/edit-sales-owner.html',
        controller: editSalesOwnerController,
        bindings: {
            resolve: '<',
            close: '&',
            dismiss: '&',
    ...
    editSalesOwnerController.$inject = ['salesOwnerService', 'adminUtility'];
    function editSalesOwnerController(salesOwnerService, adminUtility) {
        var $ctrl = this;

        $ctrl.save = function () {
            adminUtility.validateForm($ctrl.addEditForm);
            if ($ctrl.working.id) {
                salesOwnerService.update($ctrl.working).then(function (response) {   // Update 
                    $ctrl.close({ $value: $ctrl.working });
                }, processErrors)
            } else {
                salesOwnerService.create($ctrl.working).then(function (response) {   // Create 
                    $ctrl.close({ $value: $ctrl.working });
                }, processErrors)
            }
        };
